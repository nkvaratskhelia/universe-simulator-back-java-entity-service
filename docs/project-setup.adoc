...
== Code Coverage
`jacoco` plugin is used to analyze test coverage. Gradle automatically runs the `jacocoTestReport`
task after executing tests. You can also run the task manually with the following command:
`./gradlew jacocoTestReport`. The generated report can be viewed by opening the
